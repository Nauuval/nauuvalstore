...
function renderProducts() {
  const filtered = currentFilter === "all" ? allProducts : allProducts.filter(p => p.kategori === currentFilter);
  ...
  grid.innerHTML = filtered.map(product => {
    ...
    return "<div class=product-card visible>" +
      ...
      "<button class=btn-order " + disabled + " onclick=\"orderProduct('" + product.nama.replace(/'/g, "\\'") + "', " + product.harga + ')"' + ">" + btnText + "</button></div>";
  }).join("");
}

function orderProduct(nama, harga) {
